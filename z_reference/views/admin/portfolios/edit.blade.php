...
<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-primary">Edit Portfolio</h1>
        <p class="text-secondary">Update informasi project kamu.</p>
    </div>

    <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data" class="bg-surface border border-border rounded-xl p-6 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4" 
                          class="w-full bg-background border border-border text-primary rounded-lg px-4 py-3 focus:outline-none focus:border-primary placeholder-secondary/50">{{ old('description', $portfolio->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    @if($portfolio->image_path)
                        <div class="mb-3">
                            <img src="{{ Storage::url($portfolio->image_path) }}" alt="Current Image" class="w-32 h-20 object-cover rounded border border-border">
                            <p class="text-xs text-secondary mt-1">Current Image</p>
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-secondary mt-1">Leave empty to keep current image. Max 2MB.</p>
                ...
                <div>
                    ...
                    <input type="url" name="demo_url" value="{{ old('demo_url', $portfolio->demo_url) }}" 
                           class="w-full bg-background border border-border text-primary rounded-lg px-4 py-3 focus:outline-none focus:border-primary placeholder-secondary/50">
            ...
            <!-- Tech Stack -->
            <div x-data="{ tags: {{ json_encode($portfolio->tech_stack ?? []) }}, input: '' }" class="space-y-2">
            ...
            <div>
                ...
                <input type="number" name="display_order" value="{{ old('display_order', $portfolio->display_order) }}" 
