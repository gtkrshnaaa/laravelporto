...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-primary mb-2">Add New Testimonial</h2>
            <p class="text-secondary">Add a client testimonial.</p>
        </div>

        <form action="{{ route('admin.testimonials.store') }}" method="POST" class="space-y-6" x-data="{ rating: {{ old('rating', 5) }} }">
            @csrf

            <div class="space-y-6">
                <div>
                    <label for="client_name" class="block text-sm font-medium text-secondary mb-2">Client Name *</label>
                    <input id="client_name" type="text" name="client_name" value="{{ old('client_name') }}" required autofocus
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="client_position" type="text" name="client_position" value="{{ old('client_position') }}" required
                            class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none">
                    ...
                    <div>
                        ...
                        <input id="client_company" type="text" name="client_company" value="{{ old('client_company') }}" required
                            class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none">
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="5" required
                        class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none"
                        placeholder="What did the client say about your work?">{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center gap-4 pt-4">
                <button type="submit" class="bg-primary text-background font-bold py-3 px-6 rounded-lg hover:opacity-90 transition-opacity">Add Testimonial</button>
