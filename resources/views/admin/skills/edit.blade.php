...
@section('content')
    <div class="max-w-2xl">
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-primary mb-2">Edit Skill</h2>
            <p class="text-secondary">Update skill information.</p>
        </div>

        <form action="{{ route('admin.skills.update', $skill) }}" method="POST" class="space-y-6" x-data="{ level: {{ old('level', $skill->level) }} }">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input id="name" type="text" name="name" value="{{ old('name', $skill->name) }}" required autofocus
                ...
                <div>
                    ...
                    <input id="category" type="text" name="category" value="{{ old('category', $skill->category) }}" required list="category-list"
                        class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none placeholder-secondary"
            ...
            <div class="flex items-center gap-4 pt-4">
                ...
                <form action="{{ route('admin.skills.destroy', $skill) }}" method="POST" class="ml-auto" onsubmit="return confirm('Are you sure you want to delete this skill?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="text-red-500 hover:text-red-400 text-sm font-medium">Delete Skill</button>
                </form>
            </div>
        </form>
    </div>
