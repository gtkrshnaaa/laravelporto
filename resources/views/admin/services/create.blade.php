...
@section('content')
    <div class="max-w-4xl">
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-primary mb-2">Add New Service</h2>
            <p class="text-secondary">Add a new service offering.</p>
        </div>

        <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-6" x-data="{
            features: [],
            newFeature: '',
            addFeature() {
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input id="price_start" type="number" name="price_start" value="{{ old('price_start') }}" required min="0"
                        class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none">
                    @error('price_start')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input id="icon_class" type="text" name="icon_class" value="{{ old('icon_class') }}" placeholder="code, palette, chat"
                        class="w-full bg-background border border-border rounded-lg px-4 py-3 text-primary focus:ring-1 focus:ring-primary focus:border-primary transition-colors outline-none">
                    @error('icon_class')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" id="is_popular" name="is_popular" value="1" {{ old('is_popular') ? 'checked' : '' }} class="w-4 h-4 rounded border-border bg-background text-primary focus:ring-0">
            ...
            <div class="flex items-center gap-4 pt-4">
                <button type="submit" class="bg-primary text-background font-bold py-3 px-6 rounded-lg hover:opacity-90 transition-opacity">Create Service</button>
